...
pub fn variance_simd(xs: &[f32]) -> f32 {
    if xs.is_empty() {
        return 0.0;
    }

    // One-pass for mean then one-pass for variance for numerical sanity.
    let mean = sum_simd(xs) / xs.len() as f32;
    let mut acc = 0.0f32;
    let mut i = 0usize;
    const LANES: usize = 8;
...
pub fn pearson_r_simd(x: &[f32], y: &[f32]) -> f32 {
    let n = x.len().min(y.len());
    if n == 0 {
        return 0.0;
    }

    let mean_x = sum_simd(&x[..n]) / n as f32;
    let mean_y = sum_simd(&y[..n]) / n as f32;
...
pub fn cpu_correlations(telemetry: &TelemetryColumnar) -> [f32; 3] {
    ...
    let r_t = pearson_r_simd(cpu, telemetry.timestamp);
    let r_f = pearson_r_simd(cpu, telemetry.entropy_flux);
    let r_e = pearson_r_simd(cpu, telemetry.entanglement_map);
    [r_t, r_f, r_e]
}
...
pub fn select_top2_variance(telemetry: &TelemetryColumnar) -> (&'static str, &'static str) {
    ...
    let candidates: [(&'static str, f32); 4] = [
        ("timestamp", variance_simd(telemetry.timestamp)),
        ("cpu_utility", variance_simd(telemetry.cpu_utility)),
        ("entropy_flux", variance_simd(telemetry.entropy_flux)),
        ("entanglement_map", variance_simd(telemetry.entanglement_map)),
    ];
